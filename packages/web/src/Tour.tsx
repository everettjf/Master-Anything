...
import { fetchTour, narrateTourStep, type TourStep } from "./api.js";
...
export function Tour({ repoId, onClose }: { repoId: string; onClose: () => void }) {
  ...
  if (!step)
    return (
      <div className="tour">
        <div className="hint">Loading tour…</div>
      </div>
    );
      ...
      <div className="tour-card">
        ...
        <div className="tour-rel">
          {step.buildsOn.length > 0 && (
            <div>
              <span className="rel-label">builds on</span>
              {step.buildsOn.map((t) => (
                <span key={t} className="cite">
                  {t}
                </span>
              ))}
            </div>
          ...
          {step.usedBy.length > 0 && (
            <div>
              <span className="rel-label">used by</span>
              {step.usedBy.map((t) => (
                <span key={t} className="cite">
                  {t}
                </span>
        ...
        <div className="tour-nav">
          <button className="ghostbtn" disabled={i === 0} onClick={() => setI(i - 1)}>
            ← Prev
          </button>
