...
/**
 * Where to write the synthesized test. Python tests go at the repo root so pytest
 * collects them top-level (never importing the source's package, whose __init__
 * may be unimportable from a fresh clone); they load the source by path. JS/TS
 * tests sit beside the source and import it with a co-located relative path.
 */
function placeTest(language: SupportedLanguage, file: string, name: string): string {
  return language === "python" ? name : placeBeside(file, name);
}
...
function pyTest(qualname: string, sourceRel: string, cases: RawCase[]): string {
  ...
  // Load the source file directly by path (relative to this test) rather than by a
  // package import. This mirrors the capture harness and is layout-independent: it
  // bypasses the package's __init__ — which may be unimportable from a fresh clone
  // (e.g. a setuptools_scm `_version` that only exists after install) — and avoids
  // pytest resolving the module under its package name. The test lives at the repo
  // root so pytest collects it top-level, never triggering the package import.
...
${pytestImport}_spec = _ilu.spec_from_file_location("ma_char_target", _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), ${JSON.stringify(sourceRel)}))
...
export async function characterize(opts: CharacterizeOptions): Promise<Characterization | null> {
  ...
  try {
    ...
    if (opts.language === "python") {
      ...
      emit = (cases) => pyTest(opts.symbol, opts.file, cases);
    ...
    return {
      testPath: placeTest(opts.language, opts.file, testFileName(opts.language, opts.symbol)),
