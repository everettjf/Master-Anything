...
function pyTest(qualname: string, moduleName: string, cases: RawCase[]): string {
  ...
  const asserts = cases.map((c) => `    assert ${call}(*${c.args}) == ${c.val}`).join("\n");
...
function nodeTest(
  ...
  const asserts = cases.map((c) => `  assert.deepStrictEqual(${call}(...${c.args}), ${c.val});`).join("\n");
  const importLine =
    language === "typescript"
